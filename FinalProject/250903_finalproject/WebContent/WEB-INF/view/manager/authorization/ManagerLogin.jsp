...
<body>
	...
	<div class="login-container">
		...
		<div class="login-box">
			<h2 class="login-title">관리자용 로그인</h2>
			...
			<form action="UserMain.jsp" id="managerLoginForm"
				onsubmit="return validateLogin(event)">
				<div class="form-group">
					<label class="form-label" for="managerId">아이디</label>
					<div class="form-input-wrapper">
						<input type="text" id="managerId" name="managerId" class="form-input"
	...
	<script>
		...
		function validateLogin(event) {
			...
			// 관리자 로그인 요청
			processLoginManager(managerId, password);
		...
		// 로그인 처리
		function processLoginManager(managerId, password) {
			// 실제로는 서버로 데이터 전송
			console.log('로그인 시도:', managerId);

			// 관리자 로그인
			if (managerId === 'admin' && password === 'admin123') {
				alert('로그인되었습니다!');
				window.location.href = '../main/ManagerMain.jsp';
				return;
			}
		...
		document.getElementById('password').addEventListener(
				...
				function(e) {
					if (e.key === 'Enter') {
						document.getElementById('managerLoginForm').dispatchEvent(
								new Event('submit'));
					}
				});
