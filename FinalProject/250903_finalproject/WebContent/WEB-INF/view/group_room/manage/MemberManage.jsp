...
<body>
    ...
    <div class="container">
        <div class="page-header">
            <div class="header-top">
                ...
                <button class="btn-back" onclick="location.href='managelist.do?groupApplyCode=${groupApplyCode}'">← 뒤로 가기</button>
            </div>
        </div>
        ...
        <div class="content-section">
            ...
            <div id="members-tab" class="tab-content active">
                ...
                <div class="stats-grid">
                    ...
                    <div class="stat-card">
                        ...
                        <div class="stat-label">모임 정원 (모임장 제외)</div>
                    </div>
                </div>
                        ...
                        <c:if test="${member.position ne '모임장'}">
                            ...
                            <div class="member-item">
                                ...
                                <div class="member-info">
                                    <div class="member-name">
                                        ...
                                        <c:choose>
                                            ...
                                            <c:when test="${member.position eq '도우미'}">
                                                <span class="member-role role-helper">🌟 ${member.position}</span>
                                            </c:when>
                                            <c:otherwise>
                                    ...
                                    <div class="member-stats">
                                        ...
                                        <c:if test="${not empty member.totalActivity}">
                                            <span>🎯 활동: ${member.attendanceActivity}/${member.totalActivity}</span>
                                        </c:if>
                                    </div>
                                ...
                                <div class="member-actions">
                                    <button class="btn-small btn-secondary" onclick="viewMemberDetail('${member.joinCode}')">상세보기</button>
                                    <button class="btn-small btn-primary" onclick="changeRole('${member.joinCode}', '${member.nickName}')">권한 변경</button>
                                    <button class="btn-small btn-danger" onclick="kickMember('${member.joinCode}', '${member.nickName}')">퇴출</button>
                                </div>
                            </div>
                        </c:if>
                    </c:forEach>
            ...
            <div id="roles-tab" class="tab-content">
                        ...
                        <c:forEach var="member" items="${memberList}">
                            <c:if test="${member.position eq '부모임장'}">
                                <div class="member-item">
                                    ...
                                    <div class="member-info">
                                        <div class="member-name">${member.nickName}<span class="member-role role-subleader">⭐ 부모임장</span></div>
                                        <div class="member-stats">
                                            <span>📧 ${member.email}</span>
                                            <span>📅 가입일: ${member.joinDate}</span>
                                        </div>
                                    ...
                                    <div class="member-actions">
                                        <button class="btn-small btn-danger" onclick="revokeRole('${member.joinCode}', '${member.nickName}', 'subleader')">권한 해제</button>
                                    </div>
                                </div>
                            </c:if>
                        </c:forEach>
                        <c:if test="${subleaderCount == 0}">
                            <p style="color:#999; text-align:center; padding:20px;">부모임장이 없습니다.</p>
                        </c:if>
                    </div>
                </div>
                        ...
                        <c:forEach var="member" items="${memberList}">
                            <c:if test="${member.position eq '도우미'}">
                                <div class="member-item">
                                    ...
                                    <div class="member-info">
                                        <div class="member-name">${member.nickName}<span class="member-role role-helper">🌟 도우미</span></div>
                                        <div class="member-stats">
                                            <span>📧 ${member.email}</span>
                                            <span>📅 가입일: ${member.joinDate}</span>
                                        </div>
                                    ...
                                    <div class="member-actions">
                                        <button class="btn-small btn-danger" onclick="revokeRole('${member.joinCode}', '${member.nickName}', 'helper')">권한 해제</button>
                                    </div>
                                </div>
                            </c:if>
                        </c:forEach>
                        <c:if test="${helperCount == 0}">
                            <p style="color:#999; text-align:center; padding:20px;">도우미가 없습니다.</p>
                        </c:if>
                    </div>
                </div>
                ...
                <c:forEach var="member" items="${memberList}">
                    <c:if test="${member.position ne '모임장' && member.position ne '부모임장' && member.position ne '도우미'}">
                        ...
                        <c:forEach var="member" items="${memberList}">
                            <c:if test="${member.position ne '모임장' && member.position ne '부모임장' && member.position ne '도우미'}">
                                <div class="member-item">
                                    ...
                                    <div class="member-actions">
                                        <button class="btn-small btn-primary" onclick="grantRole('${member.joinCode}', '${member.nickName}')">권한 부여</button>
                                    </div>
                                </div>
                            </c:if>
                        </c:forEach>
                        <c:if test="${regularMemberCount == 0}">
                            <p style="color:#999; text-align:center; padding:20px;">일반 모임원이 없습니다.</p>
                        </c:if>
                    </div>
                </div>
            </div>

            <!-- ========== 3️⃣ 모임장 이양 탭 (다른 모임 모임장 제외) ========== -->
                        ...
                        <c:forEach var="member" items="${memberList}">
                            ...
                            <c:if test="${member.position ne '모임장'}">
                                ...
                                <div class="member-item">
                                        ...
                                        <div class="member-name">
                                            ...
                                            <c:choose>
                                                <c:when test="${member.position eq '부모임장'}">
                                                    ...
                                                    <span style="color:#f57c00; font-size:12px; font-weight:600;">⭐ 추천</span>
                                                </c:when>
                                                <c:when test="${member.position eq '도우미'}">
                                    ...
                                    <div class="member-actions">
                                        <button class="btn-small btn-primary" onclick="selectTransferTarget('${member.joinCode}', '${member.nickName}')">이양 대상 선택</button>
                                    </div>
                                </div>
                            </c:if>
                        </c:forEach>
    ...
    <div id="kickMemberModal" class="modal">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p style="margin-bottom:15px;color:#666;">퇴출 사유를 작성해주세요.</p>
                <div class="form-group">
    ...
    <script>
        ...
        function confirmKickMember() {
            ...
            if (confirm('정말 ' + currentMemberName + '님을 퇴출하시겠습니까?')) {
