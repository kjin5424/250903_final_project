	...
	<div class="container">
		...
		<div id="surveyScreen">
			<div class="survey-card">
				<div class="section">
					...
					<div class="report-info-box">
						<div class="report-info-row">
							<span class="report-label">신고 일자:</span> <span
								class="report-value">2024-10-20 14:30</span>
						</div>
						<div class="report-info-row">
							<span class="report-label">피신고자:</span> <span
								class="report-value">김문제 (닉네임 비공개)</span>
						</div>
						<div class="report-info-row">
							<span class="report-label">신고 사유:</span> <span
								class="report-category">악의적 활동(욕설/비방/모욕/괴롭힘)</span>
						</div>
					</div>
				</div>
				...
				<!-- ✅ 수정: 실제 서버로 전송되는 form -->
				<form action="reportmembersurvey.do" method="post" id="surveyForm">
					<div id="surveyScreen">
						<div class="survey-card">

							<form action="${cp}/reportmembersurvey.do" method="post"
								id="surveyForm">
								<!-- 유저코드 & 그룹적용코드 (백엔드 세션 or 파라미터로 전달) -->
								<input type="hidden" name="userCode" value="${userCode}">
								<input type="hidden" name="groupApplyCode"
									value="${groupApplyCode}">
								...
								<div class="vote-section">
									<div class="vote-question">해당 회원의 모임 탈퇴에 동의하시나요?</div>
									<div class="vote-options">
										<div class="vote-option">
											<input type="radio" name="vote" id="agree" value="agree"
												class="vote-radio"> <label for="agree"
												class="vote-label">
										...
										<div class="vote-option">
											<input type="radio" name="vote" id="disagree"
												value="disagree" class="vote-radio"> <label
												for="disagree" class="vote-label">
												<div class="vote-icon">❌</div>
												<div>동의하지 않습니다</div>
											</label>
										</div>
								...
								<div class="section">
									<div class="vote-section">
										...
										<div class="vote-options">
											<div class="vote-option">
												<input type="radio" name="surveyAnswerType" id="agree"
													value="1" class="vote-radio"> <label for="agree"
													class="vote-label">
													<div class="vote-icon">✅</div>
													<div>동의합니다</div>
												</label>
											...
											<div class="vote-option">
												<input type="radio" name="surveyAnswerType" id="disagree"
													value="2" class="vote-radio"> <label for="disagree"
													class="vote-label">
													<div class="vote-icon">❌</div>
													<div>동의하지 않습니다</div>
												</label>
										...
										<div id="agreeDetail" class="detail-section">
											...
											<!-- 											<textarea id="agreeTextarea" class="detail-textarea"
												placeholder="탈퇴 동의 사유를 구체적으로 작성해주세요. (선택사항, 최대 500자)"
												maxlength="500"></textarea> -->

											<textarea id="agreeTextarea" name="detailContent"
												class="detail-textarea"
												placeholder="탈퇴 동의 사유를 구체적으로 작성해주세요. (선택사항, 최대 500자)"
												maxlength="500"></textarea>

											<div class="char-count">
												<span id="agreeCount">0</span> / 500자
											</div>
											<div class="detail-note">ℹ️ 작성하신 내용은 신고 처리에 참고자료로
												활용됩니다. 구체적인 사례나 목격한 상황을 작성해주시면 더욱 공정한 판단에 도움이 됩니다.</div>
										</div>

										<!-- 상세 내용 입력 영역 - 비동의 -->
										<div id="disagreeDetail" class="detail-section">
											<div class="detail-title">📝 비동의 사유를 작성해주세요</div>
											<!-- 											<textarea id="disagreeTextarea" class="detail-textarea"
												placeholder="탈퇴 비동의 사유를 구체적으로 작성해주세요. (선택사항, 최대 500자)"
												maxlength="500"></textarea> -->

											<textarea id="disagreeTextarea" name="detailContent"
												class="detail-textarea"
												placeholder="탈퇴 비동의 사유를 구체적으로 작성해주세요. (선택사항, 최대 500자)"
												maxlength="500"></textarea>
											<div class="char-count">
												<span id="disagreeCount">0</span> / 500자
											</div>
											<div class="detail-note">ℹ️ 작성하신 내용은 신고 처리에 참고자료로
												활용됩니다. 신고 내용이 사실과 다르거나 오해의 소지가 있다고 판단되는 이유를 작성해주세요.</div>
										</div>
								...
								<div class="warning-message">
									...
									<div class="warning-message-content">
										...
										• 피신고자는 탈퇴 후 <strong>소명 신청</strong>을 할 수 있습니다.<br> • 허위
										또는 악의적인 투표는 제재 대상이 될 수 있습니다.
									</div>
								</div>
								...
								<div class="button-group">
									...
									onclick="submitVote()" disabled>투표 제출</button> -->
									<button type="submit" class="btn btn-submit" id="submitBtn"
										disabled>투표 제출</button>
								</div>
						</div>
					</div>
					...
					<div id="resultScreen" class="result-screen">
						<div class="survey-card">
							...
							<div class="result-message">
								의견 주셔서 감사합니다.<br> 설문 결과는 익명으로 처리되며, 일정 기준 이상 동의 시<br>
								피신고자에게 처리 결과가 안내됩니다.
							</div>
							<div class="button-group">
								<button class="btn btn-submit" onclick="goToGroupHome()">모임
									홈으로</button>
							</div>
						</div>
					</div>
        ...
        function goToGroupHome() {
			...
			<script>
// ✅ 투표 라디오 선택 시 상세입력 활성화 및 제출 버튼 활성화
...
window.addEventListener('load', function() {
    fetch('<c:url value="/surveycheck.do"/>')
      .then(res => res.text())
      .then(flag => {
          if (flag.trim() === '1') {
              // 대상이면 설문 팝업 띄우기
              window.open('<c:url value="/reportmembersurvey.do"/>',
                          'surveyPopup',
